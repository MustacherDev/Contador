class Color {
    ...
    static fromCSS(rgbString){
        const match = rgbString.match(/^rgb\((?:\s*\d+\s*,){2}\s*\d+\s*\)$/);

        if (match) {
            const values = rgbString.slice(4, -1).split(",").map(Number); // Extract and convert to numbers
            const r = values[0];
            const g = values[1];
            const b = values[2];
    ...
    sat(s){
        var hsl = this.toHSL();
        return Color.fromHSL(hsl.h, s, hsl.l, hsl.a);
    }

    lig(l){
        var hsl = this.toHSL();
        return Color.fromHSL(hsl.h, hsl.s, l, hsl.a);
    }
}
