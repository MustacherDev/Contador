...
function chooseWeighted(weights){
  ...
  var randVal = randRange(0, 1)*sum;

  sum = 0;
  for(var i = 0; i < weights.length; i++){
    sum += weights[i];
    if(randVal < sum) return i;
  }
...
function easeOutBack(percentage){
  var c1 = 1.70158;
  var c3 = c1 + 1;
  
  return 1 + c3 * Math.pow(percentage - 1, 3) + c1 * Math.pow(percentage - 1, 2);
}
...
function smartIndex(index, limit, extra = 0){
...
function simpleDeterministicRandom(index, seedOffset = 0) {

    let value = (index + seedOffset) * 0.0001; // Scale down the index to avoid very large numbers too quickly

    const fractionalPart = Math.abs(Math.sin(value * 43758.5453123));

    return fractionalPart - Math.floor(fractionalPart);
}